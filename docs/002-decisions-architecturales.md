...
**Prérequis mesuré, pas supposé** : l'encadrement lexicographique n'équivaut à une comparaison de dates que si tout `date_article` est en ISO 8601. Vérifié en production avant d'écrire une ligne de code — sur 542 articles, **0 valeur nulle, 0 non analysable par `strftime`, 0 hors format ISO**. La normalisation de l'ADR D09 avait fait son travail. C'est la quatrième fois de suite (P1, P2, la clôture des dettes de P3, ici) qu'une mesure préalable évite d'écrire du code défensif contre un problème imaginaire.
